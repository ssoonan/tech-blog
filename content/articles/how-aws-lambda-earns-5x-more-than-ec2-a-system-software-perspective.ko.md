...
그렇다면 여기서 또 다른 의문이 생깁니다. 이 글에서는 편의상 "vCPU 1개 = 물리 코어 1개"로 가정했지만, 실제로 VM은 물리 서버와 같은 것일까요? vCPU는 정말 우리만의 CPU일까요? 그리고 오버프로비저닝은 실제로 어떤 문제를 일으킬까요? [다음 글](/ko/articles/is-a-vm-really-a-server/)에서 이 구조를 더 깊게 파헤쳐 보겠습니다.